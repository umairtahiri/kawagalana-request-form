...
const Page43 = (props) => {
  ...
  const town = useSelector((state) => {
    return state?.app?.town;
  });
  useEffect(() => {
      ...
      <div style={centerAlignStyle}>
        <div className="page-number">
          <div className="page-number-style">
            <div style={{ marginRight: "5px" }}> 43</div>{" "}
            <i class="fa fa-arrow-right" style={{ fontSize: "12px" }}></i>{" "}
          </div>
        </div>
        <div>
          <div className="main-text-sub">Town *</div>
          <div className="sub-text">Landmark required</div>
          <input
            className="input-style"
            placeholder="Type Your answer here"
            onChange={(e) => dispatch(act.setTown(e.target.value))}
            id="url-input"
          />
          {showButton && (
